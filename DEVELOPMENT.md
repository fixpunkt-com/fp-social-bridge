...
(gegen TYPO3 14, höchste unterstützte Version), `.build-v12/` (gegen TYPO3 12.4)
bzw. `.build-v13/` (gegen TYPO3 13.4), damit die `composer.json` der Extension
selbst sauber bleibt. Eingecheckt sind nur die Tooling-Manifeste
(`.build/composer.json`, `.build-v12/composer.json` + `.build-v12/phpstan.neon`,
`.build-v13/composer.json` + `.build-v13/phpstan.neon`, sowie `phpstan.neon`,
...
```bash
# gegen TYPO3 14 (Standard-Toolchain, höchste unterstützte Version,
# phpstan.neon wird automatisch gefunden):
...
ddev exec ".build-v13/vendor/bin/phpstan analyse -c .build-v13/phpstan.neon"
```
...
`.build`-/`.build-v12`-/`.build-v13`-Toolchains.
